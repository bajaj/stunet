...
// INSERT your data (if is not already there)
   ...
   /*   	$findUser = "SELECT * FROM `chat_users_rooms` WHERE `username` = '$username' AND `room` ='$room' ";
	...
	//	$check = mysql_num_rows($getRoomUsers);
			...
			hasData("SELECT p.fname,p.lname FROM chat_users p,group_membership gm,groups g WHERE g.ID=gm.group AND gm.user=p.user_id AND g.name =".$room);
		$getRoomUsers=mysql_query("SELECT p.fname,p.lname FROM chat_users p,group_membership gm,groups g WHERE g.ID=gm.group AND gm.user=p.user_id AND g.name =".$room);
...
// Get People in chat
		if(mysql_num_rows($getRoomUsers) != $current)
		{
			...
			{
				$data['userlist'][] = $user['fname']." ".$user['lname'];
			}
